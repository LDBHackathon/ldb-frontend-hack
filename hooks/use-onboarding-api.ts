...
function useOnboardingStep<TPayload>(path: string) {
  ...
  const submit = useCallback(
    async (payload: TPayload) => {
      ...
      try {
        await apiRequest(path, { method: "PATCH", body: payload, silent: true })
        return true
      } catch (err) {
        const message =
...
export function useSubmitBusinessStep() {
  return useOnboardingStep<OnboardingBusinessPayload>("/api/onboarding/business")
}

export function useSubmitAddressStep() {
  return useOnboardingStep<OnboardingAddressPayload>("/api/onboarding/address")
}
...
export function useOnboardingStatus() {
  ...
  const load = useCallback(async () => {
    setIsLoading(true)
    try {
      const data = await apiRequest<OnboardingStatus>("/api/onboarding/status", {
        silent: true,
      })
      setStatus(data ?? null)
    } catch {
...
export function useSubmitOnboarding() {
  ...
  const submit = useCallback(async () => {
    setIsSubmitting(true)
    try {
      await apiRequest("/api/onboarding/submit", { method: "POST", silent: true })
