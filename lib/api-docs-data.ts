...
export const endpointGroups: EndpointGroup[] = [
  ...
  "password": "secure-password"
      ...
      },
      {
        ...
        path: "/auth/login",
        auth: "none",
        summary: "Signs in a returning user and sets the session cookie.",
        requestExample: `{
  "email": "amara@example.com",
  "password": "secure-password"
}`,
...
file=<PDF or image, max 10MB>`,
  ...
  },
  {
    ...
    description: "Credentials, webhook configuration, profile, and security.",
    endpoints: [
      {
        method: "GET",
      ...
      },
      {
        ...
        auth: "session-or-bearer",
        summary: "Rotates the secret key. New key returned once — copy it immediately.",
      },
      {
        method: "GET",
  ...
  },
  {
    ...
    description: "Read-only aggregates and lists, used once customers exist.",
    endpoints: [
      { method: "GET", path: "/portal/dashboard/summary", auth: "session-or-bearer", summary: "High-level counts and balances." },
      { method: "GET", path: "/portal/customers", auth: "session-or-bearer", summary: "List customers." },
      { method: "GET", path: "/portal/customers/{id}", auth: "session-or-bearer", summary: "Customer detail." },
      { method: "GET", path: "/portal/customers/{id}/statement", auth: "session-or-bearer", summary: "Customer statement." },
      { method: "GET", path: "/portal/transactions", auth: "session-or-bearer", summary: "All transactions." },
      { method: "GET", path: "/portal/transactions/summary", auth: "session-or-bearer", summary: "Aggregates (deposits, withdrawals, net position)." },
      { method: "GET", path: "/portal/transactions/recent", auth: "session-or-bearer", summary: "Recent activity." },
      {
        method: "POST",
        path: "/portal/simulate-transfer",
        auth: "session-or-bearer",
        summary: "Simulates an inbound transfer (sandbox only). Alias: POST /webhooks/dva-funding.",
        requestExample: `{
  ...
  "metadata": {}
      ...
      },
      {
        ...
        auth: "bearer",
        summary: "Retries Nomba provisioning if a customer is stuck in pending_nomba.",
      },
      { method: "POST", path: "/accounts/dedicated", auth: "bearer", summary: "Provisions a dedicated account for an existing customer." },
      { method: "GET", path: "/accounts/{account_id}", auth: "bearer", summary: "Account details." },
      { method: "GET", path: "/accounts/{account_id}/transactions", auth: "bearer", summary: "Paginated transactions." },
      { method: "GET", path: "/accounts/{account_id}/statement", auth: "bearer", summary: "Statement." },
      { method: "GET", path: "/customers/{customer_id}", auth: "bearer", summary: "Customer profile." },
      { method: "PATCH", path: "/customers/{customer_id}", auth: "bearer", summary: "Update customer." },
      { method: "POST", path: "/v1/customers", auth: "bearer", summary: "Versioned equivalent of POST /customers." },
    ],
  },
  {
    id: "payments",
    title: "Payments & Webhooks",
    description: "Inbound bank transfer → reconciliation → outbound merchant webhook.",
    endpoints: [
      {
        method: "POST",
        path: "/hooks/nomba",
      ...
      },
      {
        ...
        auth: "bearer",
        summary: "Registers the merchant's outbound webhook URL + events (developer alternative to the portal settings PUT).",
        requestExample: `{
  "url": "https://your-app.com/webhooks/ldb",
  "events": ["wallet.credited", "payment.partial"]
}`,
  ...
  },
  {
    ...
    description: "Generic Cloudinary uploads for KYB or other portal assets (PDF, JPEG, PNG, GIF, WebP, max 10MB).",
    endpoints: [
      { method: "POST", path: "/portal/files/upload", auth: "session-or-bearer", summary: "Single file (e.g. avatar photo)." },
      { method: "POST", path: "/portal/files/upload/multiple", auth: "session-or-bearer", summary: "Up to 10 files." },
      { method: "DELETE", path: "/portal/files/{public_id}", auth: "session-or-bearer", summary: "Remove a file." },
      { method: "GET", path: "/portal/files/{public_id}/info", auth: "session-or-bearer", summary: "File metadata." },
    ],
  },
  {
    id: "other",
    ...
    endpoints: [
      { method: "GET", path: "/health", auth: "none", summary: "Health check." },
      { method: "POST", path: "/merchants/register", auth: "none", summary: "Legacy merchant registration (API key only, no session)." },
      { method: "GET", path: "/merchants/me", auth: "bearer", summary: "Merchant profile via API key." },
      { method: "GET", path: "/metrics", auth: "none", summary: "Prometheus metrics." },
    ],
  },
]
