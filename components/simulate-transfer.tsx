    ...
    try {
      await apiRequest("/api/portal/simulate-transfer", {
        ...
        body: {
          ...
          amount,
          sender_name: senderBank ? SENDER_BANKS[senderBank] : undefined,
        },
      })
      toast.success("Transfer simulated — wallet should reconcile shortly")
      setOpen(false)
      onSimulated?.()
    } catch (err) {
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent side="center" className="max-w-2xl mx-auto rounded-xl overflow-hidden bg-white">
        <SheetHeader className="bg-[#07182a] px-6 py-4 text-slate-50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <SheetDescription className="text-sm text-slate-200">POST /portal/simulate-transfer</SheetDescription>
            </div>
            <div />
          </div>
        </SheetHeader>
        ...
        <div className="px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={senderBank} onValueChange={setSenderBank}>
                <SelectTrigger className="h-12 rounded-lg border border-slate-200">
                    <SelectValue placeholder="Select Bank" />
                </SelectTrigger>

                <SelectContent>
                  {Object.entries(SENDER_BANKS).map(([value, label]) => (
          ...
          <div className="mt-4 rounded-md border border-slate-200 bg-white p-4 text-sm text-slate-700">
            This hits the sandbox endpoint — no real bank transfer is made. Use an
            invalid NUBAN to test the misdirected-payment flow.
          </div>
        ...
        <SheetFooter>
          <div className="flex items-center justify-between">
            <Button variant="outline" className="rounded-lg hover:bg-white" onClick={() => setOpen(false)}>
              Cancel
            </Button>
            <Button
            ...
            >
              <Zap className="mr-2" /> {isSubmitting ? "Sending..." : "Send transfer"}
            </Button>
          </div>
        </SheetFooter>
      </SheetContent>
