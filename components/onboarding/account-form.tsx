...
export function AccountForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!result.success) {
      const nextErrors = result.error.flatten().fieldErrors as Partial<Record<keyof AccountFormValues, string[]>>
    ...
    <div className="space-y-6 rounded-[28px] bg-none ">
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div className="space-y-2">
          <label htmlFor="fullName" className="text-sm font-medium text-(--color-slate)">
            Your full name
          </label>
          <Input
            id="fullName"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            <label htmlFor="confirmPassword" className="text-sm font-medium text-(--color-slate)">
              Confirm password
            </label>
            <div className="relative">
              <Input
