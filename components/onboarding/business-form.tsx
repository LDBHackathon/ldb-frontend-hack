...
export function BusinessForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!result.success) {
      const nextErrors = result.error.flatten().fieldErrors as Partial<Record<keyof BusinessFormValues, string[]>>
    ...
    <div className="space-y-6">
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div className="space-y-2">
          <label htmlFor="businessName" className="text-sm font-medium text-(--color-slate)">
            Legal business name
          </label>
          <Input
            id="businessName"
          ...
          />
          {errors.businessName ? <p className="text-sm text-rose-600">{errors.businessName}</p> : null}
        </div>

        <div className="space-y-2">
          <label htmlFor="tradingName" className="text-sm font-medium text-(--color-slate)">
            Trading name
          ...
          <Input
            ...
            onChange={(event) => {
              setValues((current) => ({ ...current, tradingName: event.target.value }))
              setErrors((current) => ({ ...current, tradingName: undefined }))
            }}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label htmlFor="businessType" className="text-sm font-medium text-(--color-slate)">
              Business type
            </label>
            <Input
              id="businessType"
              ...
              onChange={(event) => {
                setValues((current) => ({ ...current, businessType: event.target.value }))
          ...
          <div className="space-y-2">
            <label htmlFor="industry" className="text-sm font-medium text-(--color-slate)">
              Industry
            </label>
            <Input
              id="industry"
              ...
              className="h-12 rounded-md border-slate-200 bg-slate-50 px-4 text-base text-(--color-slate)"
            />
            {errors.industry ? <p className="text-sm text-rose-600">{errors.industry}</p> : null}
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label htmlFor="registrationNumber" className="text-sm font-medium text-(--color-slate)">
            ...
            <Input
              ...
              onChange={(event) => {
                setValues((current) => ({ ...current, registrationNumber: event.target.value }))
                setErrors((current) => ({ ...current, registrationNumber: undefined }))
              }}
              placeholder="RC 1234567"
              className="h-12 rounded-md border-slate-200 bg-slate-50 px-4 text-base text-(--color-slate)"
            />
            {errors.registrationNumber ? <p className="text-sm text-rose-600">{errors.registrationNumber}</p> : null}
          </div>
          <div className="space-y-2">
            <label htmlFor="taxId" className="text-sm font-medium text-(--color-slate)">
              Tax ID (TIN)
              <span className="ml-2 text-xs text-slate-400">Optional</span>
            </label>
            <Input
              ...
              value={values.taxId}
              onChange={(event) => {
                setValues((current) => ({ ...current, taxId: event.target.value }))
