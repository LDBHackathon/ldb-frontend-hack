...
export function VerificationForm() {
    ...
    const { upload: uploadDocument, isUploading: isUploadingCac } =
        useFileUpload("/api/onboarding/documents")
    const { upload: uploadAddressDoc, isUploading: isUploadingAddress } =
        useFileUpload("/api/onboarding/documents")
    const [values, setValues] = useState<VerificationFormValues>(
        data ?? emptyValues
    )
    const [errors, setErrors] = useState<
        Partial<Record<keyof VerificationFormValues, string>>
    >({})
    ...
    const handleCacFileSelected = async (
    ...
    const handleAddressFileSelected = async (
        event: React.ChangeEvent<HTMLInputElement>
    ) => {
        const file = event.target.files?.[0]
        if (!file) return
        setAddressFileName(file.name)
        const url = await uploadAddressDoc(file, {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!result.success) {
            const nextErrors = result.error.flatten().fieldErrors as Partial<
                Record<keyof VerificationFormValues, string[]>
            >
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="rounded-md border border-cyan-100/30 bg-cyan-50/40 p-4 text-sm text-slate-700">
                Provide a director&apos;s details and upload incorporation
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                    <label className="text-sm font-medium text-(--color-slate)">
                    ...
                    >
                        ...
                        <div className="text-left">
                            <div className="font-medium">
                                {isUploadingAddress
                                    ? "Uploading…"
                                    : addressFileName || "Click to upload"}
                            </div>
                            <div className="text-xs text-slate-400">
                                {addressProofUrl
                                    ? "Uploaded"
                ...
                <div className="rounded-md border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                    <label className="flex items-start gap-3">
                        ...
                        <span>
                            ...
                            I&apos;m authorised to onboard this business. I
                            accept the{" "}
                            <a className="text-cyan-600 underline" href="#">
                                Terms of Service
                            </a>{" "}
                            and{" "}
                            <a className="text-cyan-600 underline" href="#">
                ...
                <label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                    ...
                    />
                    <span>
                        I agree to share these details with the verification
                        team.
                    </span>
                </label>
                {errors.consent ? (
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                    ...
                    <Button
                        type="submit"
                        disabled={
                            isSubmitting || isUploadingCac || isUploadingAddress
                        }
