      ...
      <Card className="border border-slate-200 bg-white p-6 md:p-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-500 font-medium">
              {credsLoading ? "Loading…" : `${keys.length} key${keys.length === 1 ? "" : "s"}`}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          {!credsLoading && !credsError && keys.length === 0 && (
            <p className="text-sm text-slate-400">No API keys found for this merchant yet.</p>
          )}

          {keys.map((key, index) => {
            const prefix = key.prefix ?? key.api_key_prefix ?? "Not available"
            return (
              <div key={key.id ?? index} className="space-y-2">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">
                  API Key {keys.length > 1 ? `#${index + 1}` : "Prefix"}
                  {key.status ? ` · ${key.status}` : ""}
                </label>
                <div className="flex gap-2">
                  <Input readOnly value={prefix} className="bg-slate-50 font-mono text-sm h-11" />
                  <Button
                    variant="outline"
          ...
          {rotatedKey && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button variant="outline" className="h-11 px-4 gap-2 text-slate-700" onClick={() => setRevealSecret((v) => !v)}>
                  <Eye className="h-4 w-4" />
                  {revealSecret ? "Hide" : "Reveal"}
                </Button>
                <Button variant="outline" className="h-11 px-4 gap-2 text-slate-700" onClick={() => copyToClipboard(rotatedKey)}>
                  <Copy className="h-4 w-4" />
                  Copy
                </Button>
              </div>
        ...
        <Button variant="outline" className="gap-2 text-slate-700" onClick={rotate} disabled={isRotating}>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder={webhookLoading ? "Loading…" : "https://your-app.com/webhooks/ldb"}
                className="h-11 flex-1"
              />
            </div>
            {!webhookLoading && !webhook && !webhookError && (
              <p className="text-xs text-slate-400">No webhook registered yet.</p>
            )}
          </div>
          ...
          {webhook?.signing_secret && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input readOnly value={webhook.signing_secret} className="bg-slate-50 font-mono text-sm h-11 flex-1" />
                <Button variant="outline" className="h-11 px-4 gap-2 text-slate-700" onClick={() => copyToClipboard(webhook.signing_secret ?? "")}>
                  <Copy className="h-4 w-4" />
                  Copy
                </Button>
              </div>
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-100">
            ...
            <Button variant="outline" onClick={test} disabled={isTesting || !webhook}>
