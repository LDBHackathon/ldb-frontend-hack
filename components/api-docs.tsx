...
const BASE_URL =
  process.env.NEXT_PUBLIC_DOCS_BASE_URL ?? "https://hackathon-api.ldbafrica.com"
    ...
    <div className="space-y-6">
      ...
      <Card className="border border-slate-200 bg-white p-6 md:p-8">
        ...
        <p className="text-sm text-slate-500 mb-6">
          Every endpoint this dashboard talks to, grouped by area. Portal routes accept a session cookie or a Bearer API key; developer routes require a Bearer key and an active merchant.
        </p>
      ...
      <Card className="border border-slate-200 bg-white p-6 md:p-8">
        <h3 className="text-sm font-bold text-slate-900 mb-4">Response format</h3>
...
function EndpointGroupCard({ group }: { group: (typeof endpointGroups)[number] }) {
  return (
      ...
      <div className="space-y-4">
        {group.endpoints.map((endpoint) => (
          ...
          >
            ...
            <p className="text-sm text-slate-600 mb-3">{endpoint.summary}</p>
            ...
            {endpoint.notes && (
              <p className="mt-2 text-xs text-slate-400">{endpoint.notes}</p>
            )}
          </div>
        ))}
      </div>
